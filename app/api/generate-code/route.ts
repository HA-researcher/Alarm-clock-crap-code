...
import { mockChallenges } from "@/lib/alarm/mockChallenges";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Determine whether to use mock based on request body or environment variable
    const useMock = bodyUseMock !== undefined ? bodyUseMock : process.env.USE_MOCK_CHALLENGE === "true";
    ...
    if (useMock) {
      ...
      return NextResponse.json({
        ...
        task: challenge.task,
        one_shot_fix: challenge.oneShotFix,
        expected_behavior: challenge.expectedBehavior,
        refactor_targets: ["可読性", "意図が伝わる関数名", "安全なソート処理"],
        code: {
